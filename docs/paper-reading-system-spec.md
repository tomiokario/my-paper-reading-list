...
| Paper Key | Rich text | ローカルフォルダ名、重複検出、診断表示を安定させるためのキー。Gmail Scholar、manual collect、GitHub import のいずれでも使う。 |
